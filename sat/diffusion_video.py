...
class SATVideoDiffusionEngine(nn.Module):
    def __init__(self, args, **kwargs):
        ...
        sampler_single_config = model_config.get("sampler_single_config", None)
        sampler_multi_prompt_config = model_config.get("sampler_multi_prompt_config", None)
        sampler_ddim_config = model_config.get("sampler_ddim_config", None)
        ...
        self.sampler_single = instantiate_from_config(sampler_single_config) if sampler_single_config is not None else None
        self.sampler_multi_prompt = instantiate_from_config(sampler_multi_prompt_config) if sampler_multi_prompt_config is not None else None
        self.sampler_ddim = instantiate_from_config(sampler_ddim_config) if sampler_ddim_config is not None else None
    ...
    def sample_single(
        ...
        **kwargs,
        ...
        samples = self.sampler_single(denoiser, randn, cond, uc=uc, scale=scale, scale_emb=scale_emb)
    ...
    def switch_adaln_layer(self, mixin_class_name):
        ...
        """
        if hasattr(self.model, 'switch_adaln_layer'):
            self.model.switch_adaln_layer(mixin_class_name)
        else:
            raise AttributeError(f"The model does not have a method named 'switch_adaln_layer'")
